...
La relación de este ecosistema es bastante simple. El programa principal \texttt{medcape.c}, el cual está programado para ejecutarse según enciende la BBB, es el encargado de realizar las inicializaciones pertinentes para que funcione el intercambio de información, de mapear ciertas regiones de memoria, de ejecutar el binario principal\footnote{Código ensamblador compilado que ejecuta el PRU, concretamente el fichero \texttt{PRUADC.p}}, así como de capturar y generar interrupciones hacia el código que ejecuta el PRU. Consta de un hilo que es el encargado de realizar y capturar las interrupciones. Previamente a que este hilo entre en modo de espera, manda una interrupción al código ensamblador indicando que se encuentra a la espera.
La interrupción se captura en el código ensamblador y se realiza el proceso de recopilación de las muestras. 

Cuando este proceso concluye, se manda una interrupción desde el código ensamblador al hilo del programa C que se encontraba en espera de capturar un evento concreto. La ejecución continua con una llamada al programa en C \texttt{mem2file.c}, encargado de acceder a las regiones de memoria de la BBB en la que el código ensamblador ha guardado las muestras, y guardarlas en un fichero (generalmente con extensión \texttt{.data}).

Es en este momento cuando entra en juego el programa desarrollado en Python, que accede a la información del fichero y gracias a un mecanismo de tuberías, el cual permite la sincronización y comunicación con otros procesos, es capaz de enviar esta información a otros dispositivos, en nuestro caso concreto, se envía a un dispositivo móvil con sistema operativo Android como veremos con más detalle más adelante.
...
\item \textbf{Registro r27}: Séptimo registro (3 bytes)
\end{itemize}
...
Según se van procesando las muestras, es necesario almacenarlas en algun sitio, para luego poder acceder a ellas desde el programa en C, del que se habla en la sección \ref{cap3:sec:lectura} (que a su vez vuelve a transmitir la información a otras vías). Es en este caso cuando entran en juego los bancos de memoria de la BBB, en la que se almacenan de forma temporal todas las muestras que se recogen.
