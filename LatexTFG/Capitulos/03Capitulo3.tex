...
{Detección de pérdida de una muestra a través del software de Saleae}

En la [Fig \ref{fig:perdida1}] se puede apreciar perfectamente, gracias al software proporcionado por Saleae, la pérdida de una muestra completa. En la figura se muestran las cuatro señales características del protocolo SPI, ya nombradas en la sección \ref{cap2:subsec:bus}. Además de esas cuatro señales, se puede apreciar otra más, (que es la que aparece primero en la figura) denominada DRDY (Data Ready). El flanco de subida de esta señal indica que el ADS tiene una muestra preparada para ser tratada. Se puede apreciar perfectamente en la figura que la señal deja de ser procesada aproximadamente hacia la mitad de la duración del primer ciclo, e incluso, que la muestra inmediatamente posterior es perdida; tras el siguiente flanco de subida del DRDY ,hay inmediatamente un flanco de bajada; la muestra se ha perdido por completo. Es una forma muy visual de ilustrar al lector, de las consecuencias de trabajar con un sistema operativo que puede quitarle a un proceso la CPU (aunque sea por pequeños instantes de tiempo) en el momento que crea oportuno. En este caso concreto, la perdida de la CPU por parte del proceso, aunque sea durante in intervalo de tiempo muy recudido, conlleva la pérdida de muestras. 


\figura{Vectorial/perdida2}{width=1\textwidth}{fig:perdida2}%
{Pérdida de tiempo producida por pérdida de control sobre la CPU}

En la [Fig \ref{fig:perdida2}] se ilustra aun con más detalle la pérdida de tiempo que produce para el proceso la interrupción de uso de la CPU por parte del sistema operativo. Concretamente se remarca en la figura el tiempo transcurrido entre el flanco de bajada de la señal DRDY hasta el flanco de subida de la misma señal. 

\figura{Vectorial/perdida3}{width=1\textwidth}{fig:perdida3}%
{Pérdida de otra muestra reflejado por el software de Saleae}

Y algo muy similar a las anteriores pérdidas se refleja perfectamente en la [Fig \ref{fig:perdida3}]. Las pérdidas de muestras reflejan un problema significativo en el programa original.
...
{Representación de la traza del fichero binario en Octave}
