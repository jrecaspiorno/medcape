...
Tras haber realizado la elección del tema en el que se basaría nuestro trabajo de fin de grado, así como tras la elección del director que dirigiría dicho trabajo, estabamos ansiosos de empezar lo antes posible e introducirnos en la materia. Nos pareció apasionante la idea de poder desarrollar software para dispositivos empotrados, y aun más, sabiendo que el campo con el que estaría íntimamente relacionado sería la medicina. Tras los sabios consejos que nos transmitió Joaquín sobre que placa concreta usar, de que hardware podríamos disponer para la realización del proyecto, así como el software que se nos podría facilitar, estabamos decididos a comenzar.

Inicialmente, Joaquín nos proporcionó un pequeño proyecto sobre el que poder comenzar a trabajar y sentar las bases de un proyecto mucho mayor. Este proyecto además podía funcionar correctamente con diversos chips de Texas Instruments,concretamente nosotros escogimos el ADS1198. Este dispositivo electrónico es el encargado de realizar el muestreo y de realizar las conversiones con un \ac{ADC}. El código proporcionado era relativamente sencillo y fácil de entender. Se trataba de un programa realizado en C capaz de leer en espacio de usuario del dispositivo (en nuestro caso la BBB). Es decir, era capaz de recibir y tratar toda la información recibida de forma continua por el ADS1198. Hasta aquí todo es relativamente sencillo; gracias al módulo del kernel (para la comunicación SPI) del Debian de la BBB y al programa de usuario en C era posible gestionar casi toda la información recibida de las muestras simuladas y realizar el tratamiento pertinente.

A continuación se muestra el \textbf{pseudocódigo} del programa original en C:
\begin{algorithm}
\SetAlgoLined
 \textit{initializeGPIO}\;
 \textit{configurateSPI}()\;
 \textit{resetADS}()\;
 \textit{setSampleRate}(1KHz)\;
 \textit{enableInternalReference}()\;
 \textit{readDataContinuously}()\;
 \textit{closeSPI}()\;
 \textit{StopReadDataContinuously}()\;
 \textit{printTrace}()\;
\end{algorithm}
...
En determinados contextos, la pérdida de cierto número de muestras puede ser despreciable y no requerir mayor atención. Sin embargo, en este contexto concreto, al tratarse de procesamiento de señales en tiempo real, la pérdida de un cierto número de muestras, por pequeña que sea, puede ser decisiva a la hora de interpretar la información. El programa inicial escrito en C registraba una pérdida de muestras, que se podía determinar fácilmente hallando la diferencia entre el número de muestras generadas y el número de muestras leídas en el espacio de usuario. 


\figura{Vectorial/perdida1}{width=1\textwidth}{fig:perdida1}%
{Ciclo completo de muestreo reflejado por el software de Saleae}

\figura{Vectorial/perdida2}{width=1\textwidth}{fig:perdida2}%
{Ampliación de la [Fig \ref{fig:perdida1}]}

En la [Fig \ref{fig:perdida1}] se puede apreciar perfectamente, gracias al software proporcionado por Saleae, un ciclo completo de muestreo. En la figura se muestran las cuatro señales características del protocolo SPI, ya nombradas en la sección \ref{cap2:subsec:bus}. Además de esas cuatro señales, se puede apreciar otra más, (que es la que aparece primero en la figura) denominada DRDY (Data Ready). El flanco de bajada de esta señal indica que el ADS tiene una muestra preparada para ser tratada. Cabe destacar que el programa original desarrollado en C hace uso únicamente de 2 canales (tal y como puede apreciarse en la figura), y sin embargo, para el desarrollo del presente proyecto se utilizan 8 canales distintos. El uso de más canales significaría mayor tiempo para recoger las muestras, y por lo tanto un riesgo mayor de sufrir pérdidas.

El ciclo completo que se muestra en la [Fig \ref{fig:perdida1}], permite ilustrar de forma visual uno de los principales problemas del programa original. 

Con el flanco de bajada del CS comienza la lectura secuencial de las muestras. El proceso de lectura concluye con el flanco de subida de la señal CS. El resto del tiempo hasta que hay otro flanco de bajada de la señal DRDY es tiempo que puede utilizarse para otros propósitos, como la transimión de las muestras, o la aplicación de algoritmos de delineación. Sin embargo, es en el segundo flanco de bajada de la señal DRDY cuando comienzan los problemas. Desde ese preciso instante hasta el flanco de bajada de la señal CS, no tiene lugar el proceso de lectura de las muestras. Todo el tiempo transcurrido entre los flancos de bajada de ambas señales es tiempo que se ha perdido.

En la [Fig \ref{fig:perdida2}] puede apreciarse una visión ampliada del problema anteriormente comentado. En esta figura puede apreciarse con mayor claridad el tiempo que tarda el programa en leer una muestra desde que esta preparada (flanco de bajada de DRDY) hasta que comienza su lectura (flanco de bajada del CS). Este tiempo se pierde fundamentalmente debido a las planificaciones del propio sistema operativo. Inmediatamente tras el flanco de bajada de la señal DRDY, debería estar en ejecución el proceso encargado de detectar que hay una nueva muestra lista, sin embargo, el sistema operativo acapara la CPU para otros procesos. Una vez transcurrido un intervalo de tiempo reducido (del orden de milisegundos), el proceso recibe la CPU por parte del sistema operativo y comienza la lectura de la muestra. El tiempo que ha transcurrido entre el flanco de bajada del DRDY, y la asignación de la CPU al proceso anteriormente nombrado, ha sido pérdido.

\figura{Vectorial/perdida3}{width=1\textwidth}{fig:perdida3}%
{Pérdida de una muestra reflejada por el software de Saleae}

En la [Fig \ref{fig:perdida3}] se ilustra con total claridad la pérdida de una muestra durante el proceso de lectura. Tras el primer flanco de bajada del DRDY se lee la muestra normalmente. Sin embargo, tras el segundo flanco de bajada de esta señal, se aprecia que en ningún momento hay flanco de bajada por parte de la señal CS. Transcurre tanto tiempo, que hay otro flanco de bajada de la señal DRDY (flanco de subida, y acto seguido de bajada), sin haberse producido la lectura de la muestra anterior; la muestra se ha perdido. Incluso tras este último flanco de bajada de la señal DRDY, ocurre algo muy similar a lo que comentabamos de la [Fig \ref{fig:perdida2}], ya que el tiempo que transcurre desde que hay una muestra lista, hasta que comienza el proceso de lectura de la misma, es bastante amplio.

Es una forma muy visual de ilustrar al lector, de las consecuencias de trabajar con un proyecto en tiempo real (en este caso, lectura de muestras en tiempo real), y con un sistema operativo, que no es en tiempo real (que no esta diseñado para este tipo de aplicaciones). 
...
Una vez estabamos decididos a mantener el sistema operativo, la solución tenía que encontrarse explorando otras posibles vías. Fue entonces, cuando se nos presentó la posibilidad de usar dos microprocesadores que tenía la BBB, cada uno denominado \ac{PRU}. Estos dos microprocesadores con los que contábamos, podían ser programados específicamente para tratar procesos de análisis y recopilación de muestras en tiempo real. De esta forma, aunque el sistema operativo decidiese ceder el uso de la CPU a otro proceso, las PRUs podrían seguir atendiendo los procesos en tiempo real, sin necesidad de requerir la CPU principal, por lo que la pérdida de muestras se reduciría totalmente gracias a estos pequeños procesadores que incorpora la placa. Se trata de dos procesadores de alta frecuencia (200MHz) de arquitectura de 32 bits que ofrecen la posibilidad a los desarrolladores de tratar con operaciones en tiempo real. En la [Fig \ref{fig:prus}]\footnote{Fuente original: \href{http://linuxgizmos.com/beaglebone-cape-eases-access-to-the-sitara-socs-pru/}{http://linuxgizmos.com/beaglebone-cape-eases-access-to-the-sitara-socs-pru/}} se puede apreciar un esquema hardware, que muestra la interaccioón de estos microprocesadores con el resto del dispositivo.

\figura{Vectorial/prus}{width=1\textwidth}{fig:prus}%
{Esquema hardware del PRU de la BBB}

A continuación se enumeran los motivos por los que escogimos recurrir a las PRUs:

\begin{itemize}
  \item Tienen acceso a los pines, así como a la memoria interna de la BBB y a los periféricos del principal procesador principal que incorpora.
...
Cada uno de los pines físicos que incorpora la placa tiene una función determinada, en la [Fig \ref{fig:pins}] \footnote{Fuente original: \href{http://www.toptechboy.com/beaglevone-black-rev-c/beaglebone-black-lesson-1-understanding-beaglebone-black-pinout/}{http://www.toptechboy.com/beaglevone-black-rev-c/beaglebone-black-lesson-1-understanding-beaglebone-black-pinout/}} puede apreciarse alguna de las funcionalidades que podría tener cada uno de los pines\footnote{En la [Fig \ref{fig:pins}] solo se ilustran algunas de las posibilidades que ofrece cada pin; todas las funcionalidades de cada uno de los pines se pueden consultar en \href{http://elinux.org/Beagleboard:Cape\_Expansion\_Headers}{http://elinux.org/Beagleboard:Cape\_Expansion\_Headers}}. La BBB dispone de dos cabeceras completas (conocidas como P8 y P9) con multitud de pines disponibles, que permiten realizar conexiones físicas mediante cableado. La leyenda de la [Fig \ref{fig:pins}] muestra las funciones, o mejor dicho, las posibles funciones de los distintos pines:
...
\item Si se desea simular una salida digital comprendida entre 0 y 3.3 \textit{V}, se pueden usar los pines PWM destacados en morado. 
...
Un \ac{DT} es una descripción del hardware de un sistema. Debería incorporar el nombre de la CPU, la configuración de memoria, y cualquier periférico (interno y externo). Un DT no deberia ser usado para describir software, a pesar de que listar los modulos harware pueda causar que estos se carguen. Es preciso recordar que los DTs son neutros en lo que se refiere al sistema operativo, es decir, no deberían incluir nada específico de Linux por ejemplo.

Un DT representa la configuración hardware como si de una jerarquía de nodos se tratase. Cada nodo puede contener propiedades y subnodos. Las propiedades se denominan arrays de bytes, los cuales pueden contener strings, números (big-endian), secuencias arbitrarias de bytes, y cualquier combinación de estos. Para intentar clarificar un concepto tan abstracto, trataremos de realizar una analogía con un ejemplo del mundo real; un DT podría visualizarse como un sistema de ficheros cualquiera, en el que los nodos serían los directorios, y las propiedades serían los ficheros.

Los DT generalmente se encuentran en un formato textual conocido como Device Tree Source (DTS) y son almacenados en ficheros con extensión \texttt{.dts}. La sintaxis DTS es como la de C, con llaves para agrupar código y punto y coma para concluir cada línea.
...
Un SoC (System-on-Chip) moderno es un dispositivo muy complejo; un DT completo podría suponer cientos de líneas de código. Situar un SoC en una placa junto a otros componentes solo hace que las cosas sean aun más complejas. Para que sea relativamente manejable, especialmente si hay dispositivos relacionados que comparten componentes, tiene sentido diferenciar los elementos comunes en ficheros con extensión \texttt{.dtsi}, para que sean incluidos desde los ficheros \texttt{.dts} que los requiriesen.
...
\begin{table}[]
\centering
\caption{Conexiones físicas necesarias para la alimentación del ADS1198}
\label{tabla1}
\begin{tabular}{|c|c|c|}
\hline
...
\item Fichero que forma parte de \texttt{medcape.c}, también desarrollado en C, denominado \texttt{mem2file.c} y encargado de la escritura de datos de memoria a un fichero externo.
\item Programa muy sencillo desarrollado en Python, y denominado \texttt{BTConnection.py} y encargado de la recepción y envio de información a otros dispositivos.
\end{itemize}

\figura{Vectorial/pru}{width=0.8\textwidth}{fig:pru}%
{Bancos de memoria del PRU \citep{exploringBBB}}
...
El programa en C consta de un hilo que es el encargado de realizar las interacciones pertinentes con el código que se ejecuta en el PRU. Este hilo entra en modo de espera indefinido hasta que reciba una interrupción por parte del código ejecutandose en el PRU indicando que las muestras estan listas y guardadas en el banco de memoria del PRU (Se ilustra más información sobre estos bancos de memoria en la [Fig \ref{fig:pru}]).
...
La comunicación SPI funciona tal y como se explica en la sección \ref{cap2:subsec:bus}. El proceso de comunicación se realiza en ensamblador también. Comienza dando el valor activo bajo a la señal Chip Select. A continuación se lee o se escribe un número específico de bits, de forma que se llama a la función SPICLK\_LOOP tantas veces como bits se quieran leer o escribir. Cuando ha concluido este proceso, se da el valor activo alto a la señal Chip Select. De esta forma se indica que ha finalizado una muestra concreta. Para más información sobre el proceso de comunicación SPI que se realiza en código ensamblador consultar el Apéndice \ref{ap1:com};
...
Según se van procesando las muestras, es necesario almacenarlas en algun sitio, para luego poder acceder a ellas desde el programa en C, del que se habla en la sección \ref{cap3:sec:lectura} (que a su vez vuelve a transmitir la información a otras vías). Es en este caso cuando entran en juego los bancos de memoria del PRU, en la que se almacenan de forma temporal todas las muestras que se recogen.

Este proceso de guardado de muestras en memoria se realiza en ensamblador. Para guardar la información de una muestra completa, se almacena la información almacenada en los registros pertinentes. Es decir, una vez todos los registros precisos están rellenos con la información de las muestras, el siguiente paso es el almacenamiento en memoria de cada uno de estos registros, que una vez guardados en posiciones de memoria consecutivas, reflejan el contenido de una muestra completa. 
...
Todo este proceso se refleja en el código ensamblador ilustrado con detalle en el Apéndice \ref{ap1:guardado}.
...
\figura{Vectorial/octave}{width=1\textwidth}{fig:octave}%
{Representación de la traza del fichero binario en Matlab}

Como ya se adelantaba en la sección \ref{cap3:sec:guardado}, se ha desarrollado un programa en C encargado de leer la información de las muestras almacenadas en el banco de memoria del PRU, para posteriormente guardarla en un fichero (se trata de un archivo binario) \footnote{Este fichero binario está limitado por software a un tamaño máximo de 50MB}. La traza del archivo binario puede ser representada mediante ciertos programas como Matlab u Octave. La [Fig \ref{fig:octave}] muestra la representación de este binario mediante el programa de escritorio Octave (versión para linux).

El programa en C está desarrollado a raíz de un ejemplo que se muestra en el cápitulo 13 del libro Exploring BeagleBone \citep{exploringBBB} y tanto ampliado como adaptado para su correcto funcionamiento en el presente proyecto.
...
Posteriormente se abre la tubería de Python (brevemente comentada en la sección \ref{cap3:sec:ecosistema})\footnote{La capacidad máxima de esta tubería es de 4KB}, y trata de enviarse la información por la misma en caso de haberse abierto correctamente. En el Apéndice \ref{ap1:lectura} se ilustra el proceso completode forma visual.
