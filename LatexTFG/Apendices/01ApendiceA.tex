...
\begin{resumen}
Este apéndice recoge en detalle los fragmentos más importantes para garantizar el correcto funcionamiento del proyecto.
\end{resumen}
...
El resultado final del DTO que se obtuvo finalmente y que es utilizado actualmente en la BBB se ilustra en el siguiente código. El código final fue comentado de modo que pudiese ser interpretado y comprendido de forma sencilla.
...
%-------------------------------------------------------------------
\section{Inicialización del ADS1198}
%-------------------------------------------------------------------
\label{ap1:ini}

En el siguiente fragmento de código ilustra el proceso de inicialización del ADS del proyecto original. Es un código relativamente sencillo y completamente funcional. Los pasos que se siguen en el código original, desde resetear el ciclo, hasta la lectura de datos en modo continuo, son replicados en código ensamblador. El objetivo final ha sido conseguir la misma correcta funcionalidad de la que gozaba el código original.
...
A continuación se muestra el código desarrollado en ensamblador, totalmente funcional, y utilizado en el presente proyecto, que imita a la perfección el funcionamiento del código original de inicialización del ADS en C.

\lstset{language={[x86masm]Assembler}}          % Set your language (you can change the language for each code-block optionally)
...
//3rd bit=============	
	...
	//Each increased number is half the speed
	MOV r2, 0x06
	//We only want 8 bits, but 1 register=32 bits
	//so we do lsl(logical shift left) to move our LSB to MSB 
	SUB r13, r14, 8
...
SEND_RDATAC:
	...
	SUB r13, r14, 8
	//We only want 8 bits, but 1 register=32 
	//so we do lsl(logical shift left) to move our LSB to MSB 
	CALL TRUNCATE
...
El proceso de comunicación SPI desarrollado en código ensamblador, se ilustra en los siguientes framentos de código. Cabe destacar que cada registro en ensamblador tiene una capacidad de 4 bytes. Debido al tamaño de cada muestra que se recoge, es necesario almacenar esta información en 7 registros. 

En el siguiente código se muestra la función SPICLK\_LOOP escrita en código ensamblador, en la cual se refleja el proceso de iteración sobre cada uno de los bits concretos, así como el uso de todos los registros necesarios para el almacenamiento de la información.
...
GET_SAMPLE: //Receive 1 sample (18 bytes)
	CLR	r30.t5		 // set the CS line low (active low)
	// Para evitar coger el 1er byte (FF) 
		MOV	r12, 200 //Numero aleatorio
		CALL DELAY_FUNCTION
...
A continuación se muestra el proceso de guardado de muestras desarrollado en código ensamblador.
...
STORE_DATA:         // store the sample value in memory
	...
	*/
	...
	//QBGE	END, r9, 0       
	//It's QBGE( r9 <= 0 ) and not QBEQ (r0 == 0)
...
A continuación se muestra el proceso de lectura de muestras desde el programa de usuario desarrollado en C.
